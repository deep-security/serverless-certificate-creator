...
class CreateCertificatePlugin {
  constructor(serverless, options) {
    ...
    this.hooks = {
      ...
      "before:deploy:deploy": this.createCertificate.bind(this),
      ...
      "after:remove:remove": this.deleteCertificate.bind(this),
    };
  ...
  initializeVariables() {
    this.serverless.cli.log(
      `Initializing variables... initialized=${this.initialized}, enabled=${this.enabled}`
    );
    if (!this.initialized) {
      this.serverless.cli.log(`Initializing vars`);
      this.enabled = this.evaluateEnabled();
      this.serverless.cli.log(`value of enabled: ${this.enabled}`);
      if (this.enabled) {
        ...
        this.region =
          this.serverless.service.custom.customCertificate.region ||
          "us-east-1";
        ...
        this.hostedZoneIds = this.serverless.service.custom.customCertificate
          .hostedZoneIds
          ? this.serverless.service.custom.customCertificate.hostedZoneIds
          : this.serverless.service.custom.customCertificate.hostedZoneId
          ? [].concat(
              this.serverless.service.custom.customCertificate.hostedZoneId
            )
          : [];
        //hostedZoneName is mapped for backwards compatibility
        this.hostedZoneNames = this.serverless.service.custom.customCertificate
          .hostedZoneNames
          ? this.serverless.service.custom.customCertificate.hostedZoneNames
          : this.serverless.service.custom.customCertificate.hostedZoneName
          ? [].concat(
              this.serverless.service.custom.customCertificate.hostedZoneName
            )
          : [];
        ...
        this.writeCertInfoToFile =
          this.serverless.service.custom.customCertificate
            .writeCertInfoToFile || false;
        this.rewriteRecords =
          this.serverless.service.custom.customCertificate.rewriteRecords ||
          false;
        this.certInfoFileName =
          this.serverless.service.custom.customCertificate.certInfoFileName ||
          "cert-info.yml";
        ...
        unsupportedRegionPrefixes.forEach((unsupportedRegionPrefix) => {
          if (this.region.startsWith(unsupportedRegionPrefix)) {
            console.log(
              `The configured region ${this.region} does not support ACM. Plugin disabled`
            );
            this.enabled = false;
          }
        });
      ...
      this.serverless.cli.log(
        `Done initializing... initialized=${this.initialized}, enabled=${this.enabled}`
      );
    }
  }
  ...
  reportDisabled() {
    return Promise.resolve().then(() =>
      this.serverless.cli.log(
        "serverless-certificate-creator: Custom domain is disabled."
      )
    );
  }
  ...
  getExistingCertificate() {
    return this.listCertificates().then((data) => {
      let existingCerts = data.CertificateSummaryList.filter(
  ...
  createCertificate() {
    ...
    return this.getExistingCertificate()
      .then((existingCert) => {
        if (existingCert) {
          this.serverless.cli.log(
            `Certificate for ${this.domain} in ${this.region} already exists with arn "${existingCert.CertificateArn}". Skipping ...`
          );
   ...
   */
  deleteCertificate() {
    this.serverless.cli.log(`in delete certificate method`);
    this.initializeVariables();
    if (!this.enabled) {
      return this.reportDisabled();
    }
  ...
  waitUntilCertificateIsValidated(certificateArn) {
    ...
    return this.acm
      .waitFor("certificateValidated", params)
      .promise()
      .then((data) => {
        this.serverless.cli.log(
  ...
  getCertificateProperty(src) {
    ...
    return this.listCertificates()
      .then(({ CertificateSummaryList }) => {
        let cert = CertificateSummaryList.filter(
